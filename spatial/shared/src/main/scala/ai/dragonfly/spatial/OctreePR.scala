...
//    squareInPlace(Math.max(0.0, Math.abs(v.z - center.z) - infNorm))
  ...
  def insert(v: Vec[3]): OctantPR

  def radialQuery(qv: Vec[3], radiusSquared: Double): NArray[Vec[3]]

}

class OctreePR(extent: Double, center:Vec[3] = Vec[3](0.0, 0.0, 0.0), maxNodeCapacity:Int = 64) {

  private var root: OctantPR = new LeafOctantPR(center, extent, maxNodeCapacity)
  ...
  def KNN(qv: Vec[3], k: Int): NArray[Vec[3]] = {
    if (this.size < k) throw IllegalArgumentException(
      s"KNN can't find $k nearest neighbors in OctreePR of size: ${this.size}."
    )
    ...
    def search(node: OctantPR): Unit = node match {
      case lo: LeafOctantPR =>
        var pi = 0
...
class MetaOctantPR(override val center:Vec[3], override val extent: Double, maxNodeCapacity:Int = 64) extends OctantPR {

  var s:Int = 0

  // Represent Octants in an array
  val nodes: NArray[NArray[NArray[OctantPR]]] = {
    val childInfNorm:Double = infNorm / 2.0
    val childExtent:Double = infNorm

    NArray[NArray[NArray[OctantPR]]](
      NArray[NArray[OctantPR]]( // - X
        NArray[OctantPR]( // - Y
          LeafOctantPR(Vec[3](center.x - childInfNorm, center.y - childInfNorm, center.z - childInfNorm), childExtent, maxNodeCapacity), // -X-Y-Z -> 0
          LeafOctantPR(Vec[3](center.x - childInfNorm, center.y - childInfNorm, center.z + childInfNorm), childExtent, maxNodeCapacity) // -X-Y+Z -> 1
        ),
        NArray[OctantPR]( // + Y
          LeafOctantPR(Vec[3](center.x - childInfNorm, center.y + childInfNorm, center.z - childInfNorm), childExtent, maxNodeCapacity), // -X+Y-Z -> 0
          LeafOctantPR(Vec[3](center.x - childInfNorm, center.y + childInfNorm, center.z + childInfNorm), childExtent, maxNodeCapacity) // -X+Y+Z -> 1
  ...
  def foreachNode(f:OctantPR => Any): Unit = {
    var xi = 0
    while (xi < 2) {
      var yi = 0
      while (yi < 2) {
  ...
  override def insert(v: Vec[3]): OctantPR = {
  ...
  override def toString: String = s"MetaOctantPR(center = ${center.show}, extent = $extent, maxNodeCapacity = $maxNodeCapacity, size = $size)"

}

class LeafOctantPR(override val center:Vec[3], override val extent: Double, maxNodeCapacity:Int = 64) extends OctantPR {
  ...
  private def split(): OctantPR = {
    val replacementNode = new MetaOctantPR(center, extent, maxNodeCapacity)
    //println(s"LeafOctantPR.split(size = $size, maxNodeCapacity = $maxNodeCapacity) -> $replacementNode")
  ...
  override def toString: String = s"LeafOctantPR(center = ${center.show}, extent = $extent, maxNodeCapacity = $maxNodeCapacity, size = $size)"
